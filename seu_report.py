...
def report(sess, province, city, district, lat, lon, username):
    ...
    raw_personal_info = json.loads(get_personal_info.text)['datas']['getMyDailyReportDatas']['rows'][0]
    ...
    post_info['DZ_ZDYPJG'] = ''
    post_info['DZ_JSDTCJTW'] = round(random.uniform(36, 36.4), 1)
    if district != '':
        post_info['DZ_DQWZ_WD'] = lon
...
def seu_report(username, password, province, city, district, lat, lon):
